...
function App() {
  ...
  useEffect(() => {
    ...
    if (!lang)
      watchRef.current = client
        .service("comment")
        .watch()
        .find()
        .subscribe((data) => setComments(data.reverse()));
    else
      watchRef.current = client
        .service("comment")
        .watch()
  ...
  return (
    <Container maxWidth="sm">
      <Grow in={true} timeout={1000}>
        <h1 className={classes.pageTitle} id="page-title">
          Negative <br /> Network
        </h1>
      </Grow>

      <Grid direction="row" wrap="nowrap" justify="center" container>
